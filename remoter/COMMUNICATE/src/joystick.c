...
enum dir_e getJoystick1Dir(u8 mode)
{
    ...
    else if (adcValue.thrust >= (jsParam->thrust.mid - DIR_MID_THRUST) && adcValue.thrust <= (jsParam->thrust.mid + DIR_MID_THRUST) && adcValue.yaw >= (jsParam->yaw.mid - DIR_MID_YAW) && adcValue.yaw <= (jsParam->yaw.mid + DIR_MID_YAW)) //摇杆离开了中间位置，现在查询摇杆是否回中
    {
        havebackToCenter = true;
        ret              = CENTER;
    }
...
enum dir_e getJoystick2Dir(u8 mode)
{
    ...
    else if (adcValue.pitch >= (jsParam->pitch.mid - DIR_MID_PITCH) && adcValue.pitch <= (jsParam->pitch.mid + DIR_MID_PITCH) && adcValue.roll >= (jsParam->roll.mid - DIR_MID_ROLL) && adcValue.roll <= (jsParam->roll.mid + DIR_MID_ROLL)) //摇杆离开了中间位置，现在查询摇杆是否回中
    {
        havebackToCenter = true;
        ret              = CENTER;
    }
